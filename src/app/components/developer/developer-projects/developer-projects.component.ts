...
export class DeveloperProjectsComponent implements OnInit {
  displayedColumns: string[] = ['id', 'title', 'clientName', 'endDate', 'daysRemaining', 'developerAmount', 'status', 'actions'];
  ...
  loadProjects() {
    ...
    if (currentUser) {
      this.projectService.getProjectsByDeveloper(currentUser.id)
        .subscribe({
          next: (projects) => {
  ...
  viewProject(projectId: number) {
    this.router.navigate(['/developer/projects', projectId]);
  }
